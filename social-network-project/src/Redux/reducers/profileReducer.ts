...
export const actions = {
  ...
  setUserStatusAC: (status: string) =>
    ({ type: 'SET_USER_STATUS', status: status as string } as const),
...
export const getUserProfileThunkCreator = (userId: number): ThunkType => async (
  dispatch,
): Promise<void> => {
  try {
...
export const getUserStatusThunkCreator = (userId: number): ThunkType => async (
  dispatch,
): Promise<void> => {
  try {
